    ...
    antennas.each_value do |positions|
      positions.each_index do |i|
        positions.each_index do |j|
          ...
          mark_antinode(*positions[i])
          mark_antinode(*positions[j])

          anti_row, anti_col = positions[i].clone

          loop do
            break unless mark_antinode(anti_row, anti_col)

            anti_row += diff_row
            anti_col += diff_col
          end

          anti_row, anti_col = positions[j].clone
